...
import { FaUser, FaEdit, FaSave, FaTimes, FaLock, FaUserNurse, FaCamera } from 'react-icons/fa';
import api from '../../utils/api';
import { useAuth } from '../../context/AuthContext';

const QUALIFICATIONS = ['B.Sc Nursing', 'M.Sc Nursing', 'GNM', 'ANM', 'Other'];
const DEPARTMENTS = ['ICU', 'OR / OT', 'ER / Emergency Room', 'Pediatrics', 'Cardiology', 'General Ward', 'Other'];
...
export default function NurseProfile() {
  ...
  const [passForm, setPassForm] = useState({ step: 'input' });
  ...
  const handleSendOTP = async () => {
    ...
    try {
      const { data } = await api.post('/auth/send-otp', { identifier, role: 'nurse', purpose: 'reset' });
  ...
  const handleVerifyAndReset = async () => {
    ...
    try {
      const identifier = user?.email || user?.mobileNumber;
      await api.post('/auth/verify-otp', { identifier, otp, role: 'nurse', purpose: 'reset' });
      await api.post('/auth/set-password', { identifier, role: 'nurse', password: newPass, purpose: 'reset' });
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-black text-white">Nursing Profile</h2>
      </div>

      <div className="card !p-8">
        <div className="flex items-center gap-6 mb-8 pb-8 border-b border-slate-800/50">
          <div className="relative">
            <div className="w-24 h-24 rounded-[32px] bg-gradient-to-br from-emerald-600 to-teal-700 flex items-center justify-center text-white text-4xl font-black shadow-glow shadow-emerald-500/20 overflow-hidden">
              {user?.profilePhoto ? (
            ...
            <div className="flex flex-col gap-1 mt-2">
              <div className="flex items-center gap-3">
                <span className="text-emerald-400 font-bold text-xs uppercase tracking-widest flex items-center gap-1.5">
                  <FaUserNurse /> {user?.department || 'Critical Care'}
                </span>
          ...
          {!editing ? (
            <button onClick={() => setEditing(true)} className="btn-secondary !py-2.5 !px-6 !text-xs !font-black !rounded-xl border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10">
              <FaEdit /> EDIT PROFILE
            </button>
          ) : (
            <div className="flex gap-2">
              <button onClick={handleSave} disabled={loading} className="btn-success !py-2.5 !px-6 !text-xs !font-black !rounded-xl shadow-[0_0_20px_rgba(16,185,129,0.2)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          {fields.map(f => {
            ...
            const options = f.name === 'department' ? DEPARTMENTS : QUALIFICATIONS;
                ...
                {editing ? (
                  isSelect ? (
                    <select 
                      ...
                      className="input !bg-slate-800/40 border-slate-800 focus:border-emerald-500/50" 
                    >
      ...
      <div className="card !p-8 border-amber-500/10">
        ...
        {showPassSection && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 mt-8 p-6 bg-slate-800/20 rounded-2xl border border-slate-800">
            {passForm.step === 'input' ? (
              <>
                <div className="text-sm font-medium text-slate-400 bg-blue-500/5 border border-blue-500/10 rounded-xl px-4 py-4">
                   🔐 An OTP will be sent to your registered {user?.email ? 'email' : 'mobile'}: <strong className="text-blue-400">{user?.email || user?.mobileNumber}</strong>
                </div>
                <button onClick={handleSendOTP} disabled={otpLoading} className="btn-success w-full !py-4 !rounded-xl !text-xs !font-black shadow-[0_0_20px_rgba(16,185,129,0.2)]">
